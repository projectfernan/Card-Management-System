...
Public Class frmTransact
    ...
    Sub fillL()
        ...
        Dim lup As Short
        rs = New Recordset
        rs = Conn.Execute("select CardType,`Key`,`Event` from tblcharging order By `Key`")
        If rs.EOF = False Then
            If rs("Event").Value = MainForm.txtEvent.Text Then
    ...
    Sub transact(ByVal Card As String)
        ...
        If rs.EOF = False Then
            ...
            If MainForm.txtEvent.Text <> rs("Event").Value Then
                Exit Sub
            End If
